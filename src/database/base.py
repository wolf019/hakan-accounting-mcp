...
from ..models.invoice_models import Customer, Invoice, LineItem, InvoiceStatus, PaymentReminder, CustomerType
from ..models.expense_models import Expense, BankTransaction, Reconciliation
    ...
    def init_database(self):
        ...
        try:
            with self.get_connection() as conn:
                ...
                # Create voucher annotations table for audit trail
                cursor.execute("""
                    CREATE TABLE IF NOT EXISTS voucher_annotations (
                        id INTEGER PRIMARY KEY AUTOINCREMENT,
                        voucher_id INTEGER NOT NULL,
                        ...
                        totp_verification_id INTEGER,
                        FOREIGN KEY (voucher_id) REFERENCES vouchers(id),
                        FOREIGN KEY (related_voucher_id) REFERENCES vouchers(id),
                        FOREIGN KEY (totp_verification_id) REFERENCES totp_verification_log(id)
                    )
                """)
                ...
                cursor.execute("""
                    CREATE TABLE IF NOT EXISTS user_totp_secrets (
                        ...
                        issuer_name TEXT DEFAULT 'Kaare Accounting',
                        is_active BOOLEAN DEFAULT TRUE,
                        backup_codes TEXT,
                        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                ...
                # Create TOTP rate limits table for tracking attempts
                cursor.execute("""
                ...
                # Create journal entry idempotency table for duplicate prevention
                cursor.execute("""
                    CREATE TABLE IF NOT EXISTS journal_entry_requests (
                ...
                cursor.execute("""
                    CREATE INDEX IF NOT EXISTS idx_journal_requests_expires_at 
                    ON journal_entry_requests(expires_at)
                """)
                ...
                # Add status and supersession columns to vouchers table
                try:
                    cursor.execute("ALTER TABLE vouchers ADD COLUMN status TEXT DEFAULT 'ACTIVE' CHECK (status IN ('ACTIVE', 'SUPERSEDED', 'VOID', 'DRAFT'))")
                except sqlite3.OperationalError:
                    pass  # Column already exists
                
                try:
                    cursor.execute("ALTER TABLE vouchers ADD COLUMN superseded_by INTEGER REFERENCES vouchers(id)")
                except sqlite3.OperationalError:
                    pass  # Column already exists
                
                try:
                    cursor.execute("ALTER TABLE vouchers ADD COLUMN security_level TEXT DEFAULT 'STANDARD' CHECK (security_level IN ('STANDARD', 'TOTP_REQUIRED'))")
                except sqlite3.OperationalError:
                    pass  # Column already exists
